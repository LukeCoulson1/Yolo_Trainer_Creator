        ...
        if images:
            ...
            if selected_image:
                ...
                if image:
                    ...
                    clear_canvas_key = f"clear_canvas_{selected_image}"
                    should_clear = st.session_state.get(clear_canvas_key, False)
                    ...
                    <div style="position: relative; display: inline-block; border: 2px solid #ddd; margin: 10px 0;">
                        <img id="dataset_image_{selected_image}" src="data:image/png;base64,{img_base64}"
                             style="display: block; max-width: 100%; height: auto; user-select: none;" />
                        <canvas id="dataset_canvas_{selected_image}"
                                style="position: absolute; top: 0; left: 0; cursor: crosshair; border: none; pointer-events: auto; z-index: 100;"
                    ...
                    <script>
                        (function() {{
                            ...
                            if (window.canvas_initialized_{selected_image}) {{
                                console.log('Canvas already initialized for {selected_image}');
                                return;
                            }}
                            window.canvas_initialized_{selected_image} = true;
                            ...
                            console.log('Initializing canvas for {selected_image}');
                            console.log('Image:', img);
                            console.log('Canvas:', canvas);
                            console.log('Status:', statusDiv);
                            
                            if (!img || !canvas || !statusDiv) {{
                                console.error('Missing elements for {selected_image}');
                                return;
                            }}
                            ...
                            // Make boxes accessible globally for undo function
                            window.boxes_{selected_image} = boxes;
                            ...
                            function setupCanvas() {{
                                ...
                                console.log('Canvas setup for {selected_image} - Display:', rect.width + 'x' + rect.height, 'Actual:', canvas.width + 'x' + canvas.height);
                                console.log('Canvas position check:', canvas.getBoundingClientRect());
                            }}
                            ...
                            // Draw a box
                            function drawBox(x, y, width, height, isTemp = false) {{
                            ...
                            canvas.addEventListener('mousedown', function(e) {{
                                e.preventDefault();
                                e.stopPropagation();
                                console.log('Mouse down detected on canvas {selected_image}');
                                console.log('Event:', e);
                                ...
                                updateStatus('Drawing box... Release mouse to save.');
                            ...
                            canvas.addEventListener('mousemove', function(e) {{
                                ...
                                console.log('Drawing at:', coords, 'Size:', width, 'x', height);
                            ...
                            canvas.addEventListener('mouseup', function(e) {{
                                ...
                                console.log('Mouse up detected on canvas {selected_image}');
                                isDrawing = false;
                                ...
                                console.log('Final box size:', width, 'x', height);
                                ...
                                if (width > 5 && height > 5) {{
                                    ...
                                    const centerX = (box.x + box.width / 2) / {img_width};
                                    const centerY = (box.y + box.height / 2) / {img_height};
                                    const normWidth = box.width / {img_width};
                                    const normHeight = box.height / {img_height};
                                    
                                    const yoloLine = `0 ${{centerX.toFixed(6)}} ${{centerY.toFixed(6)}} ${{normWidth.toFixed(6)}} ${{normHeight.toFixed(6)}}`;
                                    
                                    // Save to localStorage (JavaScript-only approach)
                                    let savedIndex = 0;
                                    while (localStorage.getItem('saved_annotation_{selected_image}_' + savedIndex)) {{
                                        savedIndex++;
                                    }}
                                    localStorage.setItem('saved_annotation_{selected_image}_' + savedIndex, yoloLine);
                                    
                                    console.log('Annotation saved to localStorage:', yoloLine);
                                    
                                    updateStatus(`✅ Box saved! Total: ${{boxes.length}}`);
                                }} else {{
                            ...
                            window.clearCanvas_{selected_image} = function() {{
                                ctx.clearRect(0, 0, canvas.width, canvas.height);
                                boxes = [];
                                window.boxes_{selected_image} = boxes;
                                
                                // Also clear saved annotations from localStorage
                                let index = 0;
                                while (localStorage.getItem('saved_annotation_{selected_image}_' + index)) {{
                                    localStorage.removeItem('saved_annotation_{selected_image}_' + index);
                                    index++;
                                }}
                                
                                updateStatus('Canvas and annotations cleared');
                                console.log('Canvas and annotations cleared');
                            }};
                            ...
                            if (img.complete) {{
                                ...
                                canvas.addEventListener('click', function(e) {{
                                    console.log('Canvas click test successful for {selected_image}');
                                    updateStatus('Canvas click detected!');
                                    
                                    // Draw a red dot at click position for testing
                                    const coords = getCanvasCoords(e.clientX, e.clientY);
                                    ctx.fillStyle = 'red';
                                    ctx.beginPath();
                                    ctx.arc(coords.x, coords.y, 5, 0, 2 * Math.PI);
                                    ctx.fill();
                                    console.log('Drew test dot at:', coords);
                                }});
                                
                                canvas.addEventListener('mouseenter', function(e) {{
                                    console.log('Mouse entered canvas {selected_image}');
                                }});
                                
                                canvas.addEventListener('mouseleave', function(e) {{
                                    console.log('Mouse left canvas {selected_image}');
                                }});
                    ...
                    <div style="margin: 20px 0; padding: 15px; background-color: #f8f9fa; border-radius: 8px; border: 1px solid #dee2e6;">
                        <h4 style="margin: 0 0 15px 0; color: #495057;">🎛️ Canvas Controls</h4>
                        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                            <button id="clear_canvas_{selected_image}" 
                                    style="background-color: #dc3545; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px;">
                                🗑️ Clear Canvas
                            </button>
                            <button id="view_annotations_{selected_image}" 
                                    style="background-color: #17a2b8; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px;">
                                📄 View Annotations
                            </button>
                            <button id="clear_annotations_{selected_image}" 
                                    style="background-color: #dc3545; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px;">
                                🗑️ Clear All Annotations
                            </button>
                            <button id="undo_last_{selected_image}" 
                                    style="background-color: #ffc107; color: black; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px;">
                                ↩️ Undo Last
                            </button>
                            <button id="export_annotations_{selected_image}" 
                                    style="background-color: #28a745; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px;">
                                💾 Export Annotations
                            </button>
                        </div>
                        <div id="annotations_display_{selected_image}" style="margin-top: 15px; padding: 10px; background-color: white; border-radius: 4px; border: 1px solid #e9ecef; min-height: 50px; font-family: monospace; font-size: 12px; white-space: pre-wrap; display: none;">
                        </div>
                        <div id="control_status_{selected_image}" style="margin-top: 10px; padding: 8px; background-color: #e7f3ff; border-radius: 4px; font-size: 14px; display: none;">
                        </div>
                    </div>
                    ...
                    (function() {{
                        // Get elements
                        const clearCanvasBtn = document.getElementById('clear_canvas_{selected_image}');
                        const viewAnnotationsBtn = document.getElementById('view_annotations_{selected_image}');
                        const clearAnnotationsBtn = document.getElementById('clear_annotations_{selected_image}');
                        const undoLastBtn = document.getElementById('undo_last_{selected_image}');
                        const exportAnnotationsBtn = document.getElementById('export_annotations_{selected_image}');
                        const annotationsDisplay = document.getElementById('annotations_display_{selected_image}');
                        const controlStatus = document.getElementById('control_status_{selected_image}');
                        const canvas = document.getElementById('dataset_canvas_{selected_image}');
                        const statusDiv = document.getElementById('coord_status_{selected_image}');
                        ...
                        function showStatus(message, isError = false) {{
                            ...
                            controlStatus.style.backgroundColor = isError ? '#f8d7da' : '#e7f3ff';
                            controlStatus.style.color = isError ? '#721c24' : '#004085';
                        ...
                        clearCanvasBtn.addEventListener('click', function() {{
                            // Use the new global clear function
                            if (typeof window.clearCanvas_{selected_image} === 'function') {{
                                window.clearCanvas_{selected_image}();
                                ...
                                if (canvas) {{
                                    const ctx = canvas.getContext('2d');
                                    ctx.clearRect(0, 0, canvas.width, canvas.height);
                                    showStatus('✅ Canvas cleared (fallback method)!');
                                }} else {{
                                    showStatus('❌ Canvas not found!', true);
                                }}
                            }}
                        }});
                        ...
                        viewAnnotationsBtn.addEventListener('click', function() {{
                            ...
                            let index = 0;
                            while (true) {{
                                const annotation = localStorage.getItem('saved_annotation_{selected_image}_' + index);
                                if (!annotation) break;
                            ...
                            if (savedAnnotations.length > 0) {{
                                annotationsDisplay.textContent = savedAnnotations.join('\\n');
                                annotationsDisplay.style.display = 'block';
                                showStatus(`📄 Showing ${{savedAnnotations.length}} annotation(s)`);
                            }} else {{
                                annotationsDisplay.textContent = 'No annotations found in browser storage.\\nNote: Only annotations created in this browser session are shown.';
                                annotationsDisplay.style.display = 'block';
                                showStatus('ℹ️ No annotations in browser storage');
                            }}
                        }});
                        
                        // Clear All Annotations
                        clearAnnotationsBtn.addEventListener('click', function() {{
                            if (confirm('Are you sure you want to clear ALL annotations for this image? This cannot be undone!')) {{
                                // Clear from localStorage
                                let index = 0;
                                let cleared = 0;
                                while (true) {{
                                    const key = 'saved_annotation_{selected_image}_' + index;
                                ...
                                // Hide annotations display
                                annotationsDisplay.style.display = 'none';
                                
                                showStatus(`✅ Cleared ${{cleared}} annotation(s) and canvas!`);
                            }}
                        }});
                        ...
                        undoLastBtn.addEventListener('click', function() {{
                            ...
                            if (lastIndex >= 0) {{
                                // Remove the last annotation from localStorage
                                localStorage.removeItem('saved_annotation_{selected_image}_' + lastIndex);
                                
                                // Also remove the corresponding box from the canvas
                                if (typeof window.boxes_{selected_image} !== 'undefined' && window.boxes_{selected_image}.length > 0) {{
                                    window.boxes_{selected_image}.pop();
                                    // Redraw canvas without the last box
                                    const canvas = document.getElementById('dataset_canvas_{selected_image}');
                                    if (canvas) {{
                                        const ctx = canvas.getContext('2d');
                                        ctx.clearRect(0, 0, canvas.width, canvas.height);
                                        window.boxes_{selected_image}.forEach(box => {{
                                ...
                                showStatus('↩️ Last annotation removed!');
                            }} else {{
                                showStatus('ℹ️ No annotations to undo');
                            }}
                        }});
                        ...
                        exportAnnotationsBtn.addEventListener('click', function() {{
                            ...
                            if (savedAnnotations.length > 0) {{
                                ...
                                const content = savedAnnotations.join('\\n');
                                const blob = new Blob([content], {{ type: 'text/plain' }});
                                const url = window.URL.createObjectURL(blob);
                                const a = document.createElement('a');
                                a.href = url;
                                a.download = '{selected_image.replace(".png", ".txt").replace(".jpg", ".txt").replace(".jpeg", ".txt")}';
                                document.body.appendChild(a);
                                a.click();
                                document.body.removeChild(a);
                                window.URL.revokeObjectURL(url);
                                
                                showStatus(`💾 Exported ${{savedAnnotations.length}} annotation(s) to file!`);
                            }} else {{
                                showStatus('ℹ️ No annotations to export');
                            }}
                        }});
                        
                        // Hover effects
                        [clearCanvasBtn, viewAnnotationsBtn, clearAnnotationsBtn, undoLastBtn, exportAnnotationsBtn].forEach(btn => {{
                    ...
                    components.html(controls_html, height=200)
                    
                    # Handle JavaScript signals for file operations
                    clear_signal = f"clear_file_annotations_{selected_image}"
                    undo_signal = f"undo_last_annotation_{selected_image}"
                    view_signal = f"request_annotations_{selected_image}"
                    
                    # Check for clear signal
                    clear_check_script = f"""
                    <script>
                    (function() {{
                        function checkSignals() {{
                            if (localStorage.getItem('{clear_signal}') === 'true') {{
                                // Send signal to Streamlit to clear the file
                                const input = document.querySelector('input[aria-label="Clear signal handler"]');
                                if (input) {{
                                    input.value = 'CLEAR_FILE';
                                    input.dispatchEvent(new Event('input', {{bubbles: true}}));
                                    input.dispatchEvent(new Event('change', {{bubbles: true}}));
                                }}
                                localStorage.removeItem('{clear_signal}');
                            }}
                            
                            if (localStorage.getItem('{undo_signal}') === 'true') {{
                                // Send signal to Streamlit to undo last annotation
                                const input = document.querySelector('input[aria-label="Undo signal handler"]');
                                if (input) {{
                                    input.value = 'UNDO_LAST';
                                    input.dispatchEvent(new Event('input', {{bubbles: true}}));
                                    input.dispatchEvent(new Event('change', {{bubbles: true}}));
                                }}
                                localStorage.removeItem('{undo_signal}');
                            }}
                            
                            if (localStorage.getItem('{view_signal}') === 'true') {{
                                // Send signal to Streamlit to get annotations
                                const input = document.querySelector('input[aria-label="View annotations handler"]');
                                if (input) {{
                                    input.value = 'VIEW_ANNOTATIONS';
                                    input.dispatchEvent(new Event('input', {{bubbles: true}}));
                                    input.dispatchEvent(new Event('change', {{bubbles: true}}));
                                }}
                                localStorage.removeItem('{view_signal}');
                            }}
                        }}
                        
                        // Check every 200ms
                        setInterval(checkSignals, 200);
                    }})();
                    </script>
                    """
                    
                    components.html(clear_check_script, height=0)
                    
                    # Hidden inputs for JavaScript communication
                    clear_signal_input = st.text_input(
                        "Clear signal handler",
                        key=f"clear_signal_{selected_image}",
                        placeholder="",
                        label_visibility="hidden"
                    )
                    
                    undo_signal_input = st.text_input(
                        "Undo signal handler", 
                        key=f"undo_signal_{selected_image}",
                        placeholder="",
                        label_visibility="hidden"
                    )
                    
                    view_signal_input = st.text_input(
                        "View annotations handler",
                        key=f"view_signal_{selected_image}",
                        placeholder="",
                        label_visibility="hidden"
                    )
                    
                    # Process signals
                    if clear_signal_input == "CLEAR_FILE":
                        try:
                            if os.path.exists(label_path):
                                with open(label_path, 'w') as f:
                                    f.write("")
                                st.success("✅ All annotations cleared from file!")
                                st.session_state[f"clear_signal_{selected_image}"] = ""
                                st.rerun()
                        except Exception as e:
                            st.error(f"❌ Error clearing annotations: {str(e)}")
                    
                    if undo_signal_input == "UNDO_LAST":
                        try:
                            if os.path.exists(label_path):
                                with open(label_path, 'r') as f:
                                    lines = f.readlines()
                                if lines:
                                    ...
                                    st.success("↩️ Last annotation removed from file!")
                                    st.session_state[f"undo_signal_{selected_image}"] = ""
                                    st.rerun()
                        except Exception as e:
                            st.error(f"❌ Error undoing annotation: {str(e)}")
                    
                    if view_signal_input == "VIEW_ANNOTATIONS":
                        try:
                            if os.path.exists(label_path):
                                with open(label_path, 'r') as f:
                                    content = f.read().strip()
                                if content:
                                    lines = content.split('\n')
                                    # Send annotations back to JavaScript
                                    annotations_js = f"""
                                    <script>
                                    (function() {{
                                        const annotationsDisplay = document.getElementById('annotations_display_{selected_image}');
                                        const controlStatus = document.getElementById('control_status_{selected_image}');
                                        if (annotationsDisplay && controlStatus) {{
                                            annotationsDisplay.textContent = `{content.replace('`', '\\`')}`;
                                            annotationsDisplay.style.display = 'block';
                                            controlStatus.textContent = '📄 Showing {len(lines)} annotation(s)';
                                            controlStatus.style.backgroundColor = '#e7f3ff';
                                            controlStatus.style.color = '#004085';
                                            controlStatus.style.display = 'block';
                                        }}
                                    }})();
                                    </script>
                                    """
                                    components.html(annotations_js, height=0)
                                else:
                                    no_annotations_js = f"""
                                    <script>
                                    (function() {{
                                        ...
                                        const controlStatus = document.getElementById('control_status_{selected_image}');
                                        if (annotationsDisplay && controlStatus) {{
                                            annotationsDisplay.textContent = 'No annotations found in file.';
                                            annotationsDisplay.style.display = 'block';
                                            controlStatus.textContent = 'ℹ️ No annotations found';
                                            controlStatus.style.backgroundColor = '#e7f3ff';
                                            controlStatus.style.color = '#004085';
                                            controlStatus.style.display = 'block';
                                        }}
                                    ...
                                    """
                                    components.html(no_annotations_js, height=0)
                            else:
                                no_file_js = f"""
                                <script>
                                (function() {{
                                    ...
                                    const controlStatus = document.getElementById('control_status_{selected_image}');
                                    if (annotationsDisplay && controlStatus) {{
                                        annotationsDisplay.textContent = 'No annotation file found.';
                                        annotationsDisplay.style.display = 'block';
                                        controlStatus.textContent = 'ℹ️ No annotation file found';
                                        controlStatus.style.backgroundColor = '#e7f3ff';
                                        controlStatus.style.color = '#004085';
                                        controlStatus.style.display = 'block';
                                    }}
                                ...
                                """
                                components.html(no_file_js, height=0)
                            
                            # Clear the signal
                            st.session_state[f"view_signal_{selected_image}"] = ""
                            ...
                            (function() {{
                                const controlStatus = document.getElementById('control_status_{selected_image}');
                                if (controlStatus) {{
                                    controlStatus.textContent = '❌ Error reading annotations: {str(e)}';
                                    controlStatus.style.backgroundColor = '#f8d7da';
                                    controlStatus.style.color = '#721c24';
                                    controlStatus.style.display = 'block';
                                }}
                            }})();
                            </script>
